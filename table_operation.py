...
while num != 0:
    if num == 1:
        cur.execute('CREATE TABLE English( id INT NOT NULL AUTO_INCREMENT, English VARCHAR(255) NOT NULL, Flag INT DEFAULT "0", PRIMARY KEY (id))')
        for i in table_list[1:]:
            cur.execute('CREATE TABLE ' + i + '('
                + i + 'VARCHAR(255) NOT NULL, id_e INT NOT NULL)'
                )
    if num == 2:
        for i in table_list:
    ...
    if num == 4:
        ...
        for item in table_list[1:]:
            cur.execute('SELECT * FROM ' + item + ' WHERE id_e = "' + id_e + '"')
            for i in cur:
                dictionary[item] = i[0]
